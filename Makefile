...
unit-code-coverage: ## Run code coverage report and output to HTML
	$(RUST_COMMAND) "--env RUST_BACKTRACE=full --env RUST_LOG=$(RUST_LOG)" "cargo llvm-cov --ignore-filename-regex=vendor/* --html --output-dir=/tmp/workspace/${BUILD_TAG}/target/llvm-cov/html"
