...
build-release: ## Build a release version of the agent
	$(UNCACHED_RUST_COMMAND) "$(BUILD_ENV_DOCKER_ARGS) --env RUST_BACKTRACE=full" "RUSTFLAGS='$(RUSTFLAGS)' BINDGEN_EXTRA_CLANG_ARGS='$(BINDGEN_EXTRA_CLANG_ARGS)' cargo build --no-default-features $(FEATURES_ARG) --manifest-path bin/Cargo.toml --release $(TARGET_DOCKER_ARG) && llvm-strip ./$(TARGET_DIR)/$(TARGET)/release/logdna-agent"
...
define publish_images
	...
	$(eval TARGET_VERSIONS := $(TARGET_TAG) $(shell if [ "$(BETA_VERSION)" = "0" ]; then echo "$(BUILD_VERSION)-$(BUILD_DATE).$(VCS_REF_BUILD_NUMBER_SHA) $(MAJOR_VERSION).$(MINOR_VERSION)"; fi))
	@set -e; \
...
publish-image-ibm: ## Publish SemVer compliant releases to icr
	$(DOCKER) tag $(REPO):$(IMAGE_TAG) $(DOCKER_IBM_IMAGE):3.5.5-nocap-${ARCH}
	$(DOCKER) push $(DOCKER_IBM_IMAGE):3.5.5-nocap-${ARCH}
