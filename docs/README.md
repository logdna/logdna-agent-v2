...
![agent version](https://img.shields.io/badge/Version-3.11.0--dev-E9FF92.svg)
...
**NOTE**: The Kubernetes manifest YAML files in this repository (and referenced in this
...
configuration YAML file. The default configuration yaml file is located at `/etc/logdna/config.yaml`. Unless noted, when an option says _List of_, this would mean a comma separated list for Environment variable and a YAML array list for YAML.
...
For backward compatibility agent v1 configuration file format is still supported. For example - upgrade to Agent v2 on Windows can re-use v1 config file from previous installation as-is. _The format of these v1 configuration parameter names are the same as the environment variables without `LOGDNA_` and in lower case._
...
- include _only_ specific log lines (with `LOGDNA_LINE_INCLUSION_REGEX`)
