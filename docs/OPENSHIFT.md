...
The agent can be installed in your cluster using a set of YAML files we provide. These files contain the minimum necessary OpenShift&reg objects and settings to run the agent. Teams should review and modify these YAML files for the specific needs of their clusters.
...
* OpenShift&reg cluster running Kubernetes&reg 1.9 or greater.
...
__NOTE__: To run as non-root, your OpenShift container must still be marked as privileged.
...
* **Differences:** The configuration is lacking a number of new OpenShift&reg objects. It also uses a removed environment variable for controlling journald monitoring, `USEJOURNALD`.
* **Upgrade Steps:**
  ...
  3. Overwrite the DaemonSet as well as create the new OpenShift&reg objects; run `oc apply -f k8s/agent-resources-openshift.yaml`.

> :warning: Exporting OpenShift&reg objects with "oc get \<resource\> -o yaml" includes extra information about the object's state. This data does not need to be copied over to the new YAML file.
...
The default configuration places all of the OpenShift&reg objects in a unique project. To completely remove all traces of the agent you need to simply delete the `logdna-agent` project within the Web UI.

__Note__: OpenShift&reg has no way to delete projects with the `oc` CLI. View OpenShift&reg's documentation for [managing projects](https://docs.openshift.com/container-platform/4.6/applications/projects/working-with-projects.html).
...
Follow OpenShift&reg's documentation for [enabling Journald on your nodes](https://docs.openshift.com/container-platform/4.6/logging/config/cluster-logging-systemd.html).
