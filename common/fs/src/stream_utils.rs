...
impl<St: Stream + Unpin, C: Fn(&St::Item) -> bool, P: FnMut() -> St> Recover<St, C, P> {
    ...
    /// returns true.
    pub fn recover(recover_check: C, producer: P) -> Recover<St, C, P> {
...
mod test {
    ...
    fn with_empty_stream() {
        ...
        let test_st = Recover::recover(should_recover, data_producer).collect::<Vec<String>>();
    ...
    fn exhaust_normal_stream() {
        ...
        let test_st = Recover::recover(should_recover, data_producer).collect::<Vec<usize>>();
        let result = tokio_test::block_on(test_st);
    ...
    fn when_multiple_recoveries() {
        ...
        let test_st = Recover::recover(should_recover, data_producer)
            .take(8)
            .collect::<Vec<usize>>();
        let result = tokio_test::block_on(test_st);
