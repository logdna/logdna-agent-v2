        ...
        loop {
            ...
            match event {
                Event::Initiate(path) => {
                    ...
                    info!("initiated {:?} to offset table with offset {} ({})", path, len, self.offsets.len());
                ...
                Event::Delete(ref path) => {
                    ...
                    info!("removed {:?} from offset table ({})", path, self.offsets.len());
                }
    ...
    fn tail(&mut self, path: PathBuf, sender: &Sender<LineBuilder>) {
        ...
        let offset = match self.offsets.get_mut(&path) {
            ...
            None => {
                ...
                match metadata(&path).map(|m| m.len()) {
                    Ok(v) => self.offsets.insert(path, v),
                    Err(e) => {
