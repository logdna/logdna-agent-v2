...
use crate::{
    offsets::OffsetMap,
    span::{Span, SpanVec},
    FileId, FileOffsetEvent, FileOffsetFlushHandle, FileOffsetShutdownHandle, FileOffsetStateError,
    FileOffsetUpdate, FileOffsetWriteHandle, FileOffsets,
};
...
mod test {
    ...
    fn new_agent_state_dir_create_error() {
        ...
        let noperm_dir = tempdir().unwrap().keep().join("denied");
        assert!(
            !noperm_dir.exists(),
            "test prereq failed: {:?} reported as already existing",
            noperm_dir
    ...
    fn new_agent_state_not_dir() {
        let blocking_file = tempdir().unwrap().keep().join("block");
