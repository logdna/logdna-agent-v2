...
pub async fn claim_lease(lease_name: String, lease_client: &Api<Lease>) {
    ...
    println!("Lease {} now owned by {:?}",lease_name, patch_lease.unwrap().spec.unwrap().holder_identity);
}
...
pub async fn release_lease(lease_name: String, lease_client: &Api<Lease>) {
    ...
    let pp = PatchParams::apply(&lease_name);
    let patch_lease = lease_client.patch(&lease_name, &pp, &patch).await;
    println!("Lease {} has been released to {:?}", &lease_name, patch_lease.unwrap().spec.unwrap().holder_identity);
}

async fn get_lease_list(lease_label: &str, lease_client: &Api<Lease>
...
mod tests {
    ...
    async fn test_leasepatchspec_object_serialize() {
        ...
        assert_eq!(test_leasepatchspec.spec.holderIdentity.unwrap(), test_expected_agent);
